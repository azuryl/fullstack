...
def search_zillow(request_url, xpath):
    ...
    #session_requests.adapters.DEFAULT_RETRIES = 3
    try:
        response = session_requests.get(request_url, headers=getHeaders(),timeout=3)
    except Exception:
        print "search_zillow Timeout"
        return{}  
    print "got response from search zillow"
    print response.url
    try:  
...
def get_property_by_zpid(zpid):
    ...
    try:
        ...
        print "get_property Timeout"
        return{}
    #print "got response from search property"
    print response.url
    try:
        tree = html.fromstring(response.content)
    except Exception:
