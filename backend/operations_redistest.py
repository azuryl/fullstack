...
def getDetailsByZpid(zpid,get_prediction=False):
    ...
    #prop = json.loads(cache.get({'zpid':zpid}))
    #print "redis"
    #print  prop
    ...
    print type(prop)
    print "json.dumps"
    propdu =json.dumps(prop)
    print propdu
    print type(propdu)

    print "set redis"
    cache.set((zpid),propdu) 
    cachprop=  json.loads(cache.get((zpid)))
    
    print "get cacheprop"
    print cachprop,type(prop),type(cachprop)
    prop = cachprop
